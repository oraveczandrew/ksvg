...
class MockCanvas {
    ...
    fun save(saveFlags: Int): Int {
        val n = saveCount
        this.operations.add(String.format(Locale.US, "save(%x) -> $n", saveFlags))
    ...
    private fun internalSave(saveFlags: Int) {
        this.matrixStack.push(
            if (saveFlags and MATRIX_SAVE_FLAG != 0) {
                Matrix(matrix)
            } else {
                null
            }
        ...
        this.clipPathStack.push(
            if (saveFlags and CLIP_SAVE_FLAG != 0) {
                Path(clipPath)
            } else {
                null
            }

            ...
            return root
                ...
                .flatMap { category ->
                    category
                        .listDirectories()
                        .flatMap { subFolder ->
                            val relativePath = subFolder.toRelativeString(root)
                            val targetSubFolder = File(METEOCONS_GOLDEN_ROOT_PATH, relativePath)
                            subFolder.listSvgs().map { svg ->
                                arrayOf(svg, targetSubFolder)
                            }
                        }
                }
        }

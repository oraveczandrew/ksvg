...
private const val DIFF_HIGHLIGHT = 0xFFFF00FF.toInt() // magenta
...
internal class BitmapComparator(
    ...
    /**
     * Writes a difference visualization into [out]: pixels that differ from [b1] are
     * highlighted in a vivid colour, while matching pixels keep the reference content.
     * [out] must have the same dimensions as [b1] and [b2].
     */
    internal fun diffBitmap(b1: Bitmap, b2: Bitmap, out: Bitmap) {
        assert(b1 !== b2)
        ...
        for (i in outPixels.indices) {
            outPixels[i] = if (isColorSimilar(pixels1[i], pixels2[i])) {
                pixels1[i]
            } else {
                DIFF_HIGHLIGHT
            }

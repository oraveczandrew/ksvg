...
/**
 * Instrumented Android validation test for feDisplacementMap.
 */
@RunWith(AndroidJUnit4::class)
class DisplacementMapNativeDeviceTest {

    private fun checkBackend(backend: Int, backendName: String) {
        assertTrue("libksvgblur not loadable on device", DisplacementMapNative.isAvailable)
        
        Log.i("DisplacementValidation", "Testing backend: $backendName")
        Log.i("DisplacementValidation", "ABI: ${Build.SUPPORTED_ABIS.joinToString()}")
    ...
    fun neon64MatchesKotlin() {
        ...
        if (abi == "arm64-v8a") {
            ...
            Log.i("DisplacementValidation", "Skipping neon64 on $abi")
        }
    }

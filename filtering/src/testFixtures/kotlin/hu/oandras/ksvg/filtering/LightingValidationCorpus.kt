...
public object LightingValidationCorpus {
    ...
    public val cases: List<Case> = buildList {
        ...
        add(Case(
            ...
            lightG = 255,
            lightB = 255,
            params = doubleArrayOf(8.0, 8.0, 100.0, 8.0, 8.0, 0.0, 30.0, 1.0),
            premultiplied = false,
            useLinear = true,
            input = UnLinearizeValidationCorpus.fixedSeedRandom(16 * 16)
        ))
        ...
        // Distant light, specular, linear output, red-dominant light color.
        // With white light R==G==B everywhere, so a wrong-channel alpha-max
        // stays invisible; here R strictly wins and alpha must equal the
        // post-L2S sRGB red (not the linear red byte). Low exponent keeps
        // intensities mid-range, where linear and sRGB bytes differ
        // (at 0/255 they coincide, hiding the bug).
        add(Case(
            name = "distant specular linear red 16x16",
            width = 16,
            height = 16,
            clipLeft = 0,
            clipTop = 0,
        ...
        // Distant light, specular, linear output, white light, low exponent.
        // Mid-range intensities exercise round-half-up intensity scaling on
        // every channel (at 0/255 round and truncate coincide, hiding the
        // difference).
        add(Case(
            name = "distant specular linear white exp2 16x16",
            width = 16,
            height = 16,
            clipLeft = 0,
            clipTop = 0,
        ...
        add(Case(
            ...
            lightG = 255,
            lightB = 255,
            params = doubleArrayOf(8.0, 8.0, 50.0, 8.0, 8.0, 0.0, 30.0, 1.0),
            premultiplied = false,
            useLinear = false,
            input = UnLinearizeValidationCorpus.fixedSeedRandom(16 * 16)
        ))
        ...
        add(Case(
            ...
            lightG = 255,
            lightB = 255,
            params = doubleArrayOf(8.0, 8.0, 50.0, 0.0, 0.0, 0.0, Double.NaN, 1.0),
            premultiplied = true,
            useLinear = false,
            input = UnLinearizeValidationCorpus.fixedSeedRandom(16 * 16)
        ))
        ...
        add(Case(
            ...
            lightG = 255,
            lightB = 255,
            params = doubleArrayOf(8.0, 8.0, 100.0, 8.0, 8.0, 0.0, 30.0, 2.0),
            premultiplied = false,
            useLinear = false,
            input = UnLinearizeValidationCorpus.fixedSeedRandom(16 * 16)
        ))
        ...
        add(Case(
            ...
            lightG = 255,
            lightB = 255,
            params = doubleArrayOf(8.0, 8.0, 100.0, 8.0, 8.0, 0.0, 30.0, 0.5),
            premultiplied = false,
            useLinear = true,
            input = UnLinearizeValidationCorpus.fixedSeedRandom(16 * 16)
        ))

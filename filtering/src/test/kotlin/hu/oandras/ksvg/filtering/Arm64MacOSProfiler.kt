...
/**
 * Apple Silicon (ARM64) macOS implementation of [HostProfiler] using the private `kpc` API
 * exposed via `profiler_macos.cpp`.
 */
class Arm64MacOSProfiler : HostProfiler {
        ...
        val isSupported: Boolean
            get() = try {
                ...
                os.contains("mac") && (arch.contains("aarch64") || arch.contains("arm64")) &&
                        Arm64MacOSProfiler().nativeIsAvailable()
            } catch (_: Throwable) {
                false
            }
    }

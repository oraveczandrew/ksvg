...
/**
 * On-device (phone/emulator) differential test for [NativeGaussianBlur].
 *
 * The native lib only loads on a real device ABI, so this instrumented test is
 * the actual hardware validation. It compares the native blur against an
 * independent Kotlin Gaussian reference (transparent-black edges,
 * stdDeviation == sigma) and fails if any pixel differs by more than a small
 * rounding tolerance.
 *
...
public class NativeGaussianBlurDeviceTest {
    ...
    private fun referenceGaussian(pixels: IntArray, w: Int, h: Int, sx: Float, sy: Float): IntArray {
        ...
        fun conv(src: FloatArray): FloatArray {
            ...
            for (y in 0..<h) for (x in 0..<w) {
                ...
                for (k in -rx..rx) {
                    val xx = x + k
                    if (xx in 0..<w) s += src[y * w + xx] * wx[k + rx]
                }
                tmp[y * w + x] = s
            }
            ...
            for (y in 0..<h) for (x in 0..<w) {
                ...
                for (k in -ry..ry) {
                    val yy = y + k
                    if (yy in 0..<h) s += tmp[yy * w + x] * wy[k + ry]
                }
                out[y * w + x] = s
            }
            return out
    ...
    public fun blurMatchesReference() {
        ...
        val cases = listOf(
            ...
            Case(16, 16, 2f, 2f),
            Case(32, 24, 4f, 4f),
            Case(48, 48, 8f, 8f), // r = 24 -> optimized kernel path
            Case(48, 48, 9f, 9f), // r = 27 -> scalar fallback
            Case(20, 30, 3f, 5f), // anisotropic -> scalar path
        )
        var overallMax = 0
        for (c in cases) {
            val rnd = Random(c.w * 1000 + c.h * 7 + (c.sx * 100).toInt())
            val px = IntArray(c.w * c.h) {
                val alpha = if (rnd.nextBoolean()) 0 else 255
                (alpha shl 24) or (rnd.nextInt(256) shl 16) or
            ...
            println(
                "NativeGaussianBlurDeviceTest ${c.w}x${c.h} sigma=(${c.sx},${c.sy}) " +
                    "maxDiff=$maxD",
            )
            assertTrue(
            ...
            if (maxD > overallMax) overallMax = maxD
        }
        println("NativeGaussianBlurDeviceTest OVERALL maxDiff=$overallMax")
    }
}
